...
dependencies_pid=
site_dependencies_pid=
dependency_stamp="$repo_root/.dakia-node-modules.sha256"
site_dependency_stamp="$repo_root/apps/site/.dakia-node-modules.sha256"
lockfile_sha256=$(shasum -a 256 "$repo_root/package-lock.json" | awk '{print $1}')
site_lockfile_sha256=$(shasum -a 256 "$repo_root/apps/site/package-lock.json" | awk '{print $1}')
dependency_fingerprint="$lockfile_sha256 $(node --version)"
site_dependency_fingerprint="$site_lockfile_sha256 $(node --version)"
installed_dependency_fingerprint=
installed_site_dependency_fingerprint=
if [ -f "$dependency_stamp" ]; then
  installed_dependency_fingerprint=$(sed -n '1p' "$dependency_stamp")
fi
if [ -f "$site_dependency_stamp" ]; then
  installed_site_dependency_fingerprint=$(sed -n '1p' "$site_dependency_stamp")
fi
...
if [ ! -x "$repo_root/apps/site/node_modules/.bin/vite" ] ||
  [ "$installed_site_dependency_fingerprint" != "$site_dependency_fingerprint" ]; then
  echo "Installing locked website dependencies..."
  npm --prefix "$repo_root/apps/site" ci &
  site_dependencies_pid=$!
fi
...
if [ -n "$site_dependencies_pid" ]; then
  if wait "$site_dependencies_pid"; then
    printf '%s\n' "$site_dependency_fingerprint" >"$site_dependency_stamp"
  else
    status=1
  fi
fi

...
export function MailList({
  ...
  const selectedCount = threads.filter((thread) =>
  ...
  return (
    <section className="mail-list-panel" aria-label={mailboxTitle}>
      ...
      <div className="mail-scroll" onScroll={loadNearEnd}>
        ...
        {loading && !syncStatus && threads.length === 0 ? (
...
function SmartThreadSections(props: RowsProps) {
  const { t } = useTranslation();
  const starred = props.threads.filter((thread) =>
    thread.messages.some((message) => message.is_flagged),
  );
  const unstarred = props.threads.filter(
    (thread) => !thread.messages.some((message) => message.is_flagged),
  );
  const sections = [
    { id: "starred", label: "inbox.starred", threads: starred },
    ...categories.map(({ id, label }) => ({
      id,
      label,
      threads: unstarred.filter(
        (thread) =>
          (thread.unread || thread.id === props.activeThreadId) &&
          (thread.latest.category ?? "other") === id,
      ),
    })),
    {
      id: "seen",
      label: "inbox.seen",
      threads: unstarred.filter(
        (thread) => !thread.unread && thread.id !== props.activeThreadId,
      ),
    },
  ];
  return sections.map(({ id, label, threads }) => {
    if (!threads.length) return null;
    return (
      <section className="smart-section" key={id} aria-label={t(label)}>

...
export function onAccountsChanged(
  handler: (accounts: Account[]) => void,
): Promise<UnlistenFn> {
  if (!isTauri()) return Promise.resolve(() => undefined);
  return listen<Account[]>("accounts-changed", (event) =>
    handler(event.payload),
  );
}
...
export async function notifyAccountsChanged(accounts: Account[]) {
  if (isTauri()) await emitTo("main", "accounts-changed", accounts);
}

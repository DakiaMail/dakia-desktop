...
impl Store {
    ...
    async fn search_conversation_matches(&self, query: &SearchQuery) -> Result<Vec<MailSummary>> {
        ...
        sql.push_str(") SELECT * FROM matching WHERE thread_rank = 1");
        if query.cursor.is_some() {
...
mod tests {
    ...
    async fn migration_groups_existing_reply_shaped_messages() {
        ...
        for (id, uid, subject) in [
            ...
            sqlx::query("INSERT INTO messages(id, account_id, mailbox, uid, message_id, subject, from_address, to_addresses, received_at, snippet, body_text) VALUES (?, 'account', 'INBOX', ?, ?, ?, 'mara@example.com', 'you@example.com', ?, 'legacy preview', 'legacy full body')")
                .bind(id)
                .bind(uid)
    ...
    async fn deleting_an_account_also_deletes_its_local_messages() {
        ...
        store.upsert_messages(&[stored_message]).await.unwrap();

...
2. run the packaged-app startup check when native to the machine;
3. notarize and staple the app;
4. rebuild the DMG from that final app;
5. notarize, staple, and mount-verify the DMG;
6. archive that same final app and sign the exact archive bytes.

The Intel app receives its actual startup and updater/restart proof on the
Intel MacBook, not under Rosetta.

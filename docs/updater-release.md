...
Store the two R2 values without adding them to shell history:
...
never write their values into the repository. The account ID is not secret and
is fixed in `scripts/local-release-env.sh`.
...
Use an older staging build as the baseline. The examples below use `v0.2.7` and
`v0.2.8`.
...
npm run release:build -- v0.2.8 staging
npm run release:publish:staging -- \
  v0.2.8 "$PWD/release-assets/v0.2.8/staging"
```
...
npm run release:test:intel-kit -- \
  v0.2.7 \
  "$PWD/release-assets/v0.2.7/baseline/Dakia_0.2.7_x64.dmg" \
  "$PWD/release-assets/v0.2.8/intel-test-kit"
```
...
npm run release:test:update -- \
  v0.2.7 v0.2.8 <mode> \
  "$PWD/release-assets/v0.2.7/baseline/Dakia_0.2.7_aarch64.dmg" \
  "$PWD/release-assets/v0.2.8/staging" \
  "$PWD/release-assets/v0.2.8/evidence"
```
...
```bash
./run-test.sh v0.2.8 <mode>
```

Copy the kit's `evidence/x86_64` directory back into
`release-assets/v0.2.8/evidence/x86_64` on the primary Mac. Then select and test
the next mode.
...
npm run release:test:verify -- \
  v0.2.8 "$PWD/release-assets/v0.2.8/evidence"
```
...
./scripts/record-local-updater-waiver.sh \
  v0.2.8 x86_64 tampered-archive \
  "$PWD/release-assets/v0.2.8/evidence" \
...
```bash
npm run release:build -- v0.2.8 production
npm run release:publish:production -- \
  v0.2.8 \
  "$PWD/release-assets/v0.2.8/production" \
  "$PWD/release-assets/v0.2.8/evidence"
```
